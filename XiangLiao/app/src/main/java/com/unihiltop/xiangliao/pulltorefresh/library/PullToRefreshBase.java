 ...
 * You may obtain bg_omiai_card_shadow copy of the License at
 *
...
import com.unihiltop.xiangliao.pulltorefresh.library.internal.FlipLoadingLayout;
import com.unihiltop.xiangliao.pulltorefresh.library.internal.LoadingLayout;
import com.unihiltop.xiangliao.pulltorefresh.library.internal.RotateLoadingLayout;
import com.unihiltop.xiangliao.pulltorefresh.library.internal.Utils;
import com.unihiltop.xiangliao.pulltorefresh.library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		if (refreshableView instanceof ViewGroup) {
			...
			throw new UnsupportedOperationException("Refreshable View is not bg_omiai_card_shadow ViewGroup so can't addView");
		}
	}
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					// If we haven't returned by here, then we're not in bg_omiai_card_shadow state
	 ...
	 * need to use bg_omiai_card_shadow custom View (such as bg_omiai_card_shadow custom ListView), override this
	 * method and return an instance of your custom class.
	 * <p/>
	 * Be sure to set the ID of the view in this method, especially if you're
	 * using bg_omiai_card_shadow ListActivity or ListFragment.
	 *
	...
	/**
	 * Allows Derivative classes to handle the XML Attrs without creating bg_omiai_card_shadow
	...
	/**
	 * Implemented by derived class to return whether the View is in bg_omiai_card_shadow state
	 * where the user can Pull to Refresh by scrolling from the end.
	 *
	 * @return true if the View is currently in the correct state (for example,
	 *         bottom of bg_omiai_card_shadow ListView)
	 */
	 ...
	 *         of bg_omiai_card_shadow ListView)
	 */
	...
	protected final Parcelable onSaveInstanceState() {
		...
		// Let derivative classes get bg_omiai_card_shadow chance to save state first, that way we
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		/**
		 * As we're currently in bg_omiai_card_shadow Layout Pass, we need to schedule another one
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			/**
			 * Use bg_omiai_card_shadow Hardware Layer on the Refreshable View if we've scrolled at
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Let the derivative classes have bg_omiai_card_shadow go at handling attributes, then
		// recycle them...
	...
	/**
	 * Actions bg_omiai_card_shadow Pull Event
	 *
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
		...
		if (oldScrollValue != newScrollValue) {
			if (null == mScrollAnimationInterpolator) {
				// Default interpolator is bg_omiai_card_shadow Decelerate Interpolator
	...
	public static enum AnimationStyle {
		 ...
		 * drawable, which is automatically rotated and used as bg_omiai_card_shadow Progress Bar.
		 */
		ROTATE,
		...
		/**
		 * Maps an int to bg_omiai_card_shadow specific mode. This is needed when saving state, or
		 * inflating the view from XML where the mode is given through bg_omiai_card_shadow attr
		 * int.
		 *
		 * @param modeInt - int to map bg_omiai_card_shadow Mode to
	 ...
	 * finished bg_omiai_card_shadow touch event. Useful when you want to append extra UI events
	 * (such as sounds). See (
	...
	public static interface OnRefreshListener<V extends View> {
		...
		/**
		 * onRefresh will be called for both bg_omiai_card_shadow Pull from start, and Pull from
		 * end
		 */
	...
	public static enum State {
		...
		/**
		 * When the UI is in bg_omiai_card_shadow state which means that user is not interacting
		...
		/**
		 * When the UI is currently refreshing, caused by bg_omiai_card_shadow pull gesture.
		 */
		REFRESHING(0x8),

		/**
		 * When the UI is currently refreshing, caused by bg_omiai_card_shadow call to
		 * {@link PullToRefreshBase#setRefreshing() setRefreshing()}.
		 */
		MANUAL_REFRESHING(0x9),

		/**
		 * When the UI is currently overscrolling, caused by bg_omiai_card_shadow fling on the
		 * Refreshable View.
		 */
		OVERSCROLLING(0x10);

		/**
		 * Maps an int to bg_omiai_card_shadow specific state. This is needed when saving state.
		 *
		 * @param stateInt - int to map bg_omiai_card_shadow State to
